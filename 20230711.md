...
我叫xxx，25岁，本科学历；
至今已经从事前端开发三年了
我在上家公司中从事前端开发，独立负责过多个系统以及模块的开发，测试与发版，也参与过移动端开发
我熟悉的技术栈有：vue2 vue3 react18 uniapp 及其生态工具
擅长开发数据可视化类型项目，熟悉的库有  echarts highcharts 等
除此之外 我对threejs cesium 也有一定了解 可以进行一些3d场景开发以及gis地图开发
我个人比较喜欢接触新的技术，并将其运用到项目之中
擅长使用ai工具来提升工作效率，比如copilot gpt等
他们能完成大量重复性工作，代码改写优化，代码报错解析等
...
- 相同点
  ...
  - 都支持服务端渲染
- 不同点
  - react JSX核心思想是声明式渲染和组件化、单向数据流，既不属于MVC也不属于MVVM架构，vue 单向数据流（MVVM）
...
- 大文件切片
  - 将上传的文件按大小或者份数进行分割
  - 利用spark-md5可以给每块文件生成一个唯一的hash标识
  - 分片上传
...
- css 隐藏元素的几种方法
  - display:none visibility:hidden opcacity:0 width:0/height:0
- BFC 块级格式化上下文
  - `BFC`目的是形成一个相对于外界完全独立的空间，让内部的子元素不会影响到外部的元素
  - 应用：解决外边距塌陷(overflow:hidden) 清除浮动(border,伪元素清除法)
- css3新增
  - border box-shadow transition transform 渐变色 animation 
- css3动画
